...
governs ``aethrionis/`` only — ``check_vault.py`` validates a tag against the
...
def apply_colour_groups(vault: Path, check: bool = False) -> dict:
    ...
    AETHRIONIS's own script, which rewrites `colorGroups` wholesale from its
...
> **DUM-E builds AETHRIONIS. AETHRIONIS does the science.** DUM-E is a temporary
> commissioning harness. When AETHRIONIS has commissioned equivalent capabilities,
> DUM-E shrinks into a thin commissioning profile or is retired. It must not
> grow into a second AETHRIONIS.
...
> `.obsidian/graph.json` is generated by AETHRIONIS's `make_vault_graph.py`, which
