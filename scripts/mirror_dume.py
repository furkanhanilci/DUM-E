...
Tags use the ``dume/`` namespace deliberately. A shared vault has other
projects' controlled vocabularies in it, and each validates tags under its own
prefix, so a separate project takes a separate namespace rather than enlarging
someone else's.
...
def apply_colour_groups(vault: Path, check: bool = False) -> dict:
    ...
    A note on ownership: another project in this vault generates the same
    file, rewriting `colorGroups` wholesale from its own static list. Running
    that generator will therefore remove these groups. The two tools are not
    merged, because merging them would make one project's tooling own the
    other's; re-running this one restores the colours.
    """
...
> DUM-E owns the glue between a work package and a merge-eligible candidate,
> and nothing else. It is not an agent framework, not a model server and not
> the project it builds — the target is bound as configuration.
...
> `.obsidian/graph.json` is also generated by another project in this vault,
> which rewrites `colorGroups` wholesale from its own static list. Running that
> generator removes these groups. `scripts/mirror_dume.py` re-adds them and preserves
