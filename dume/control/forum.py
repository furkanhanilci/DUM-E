"""AETHRION's spaces as a Telegram forum.

A forum supergroup has topics, and a topic is a conversation with its own name
and its own unread state. That is the same shape AETHRION already has, so the
...
# Name, and the AETHRION channel it mirrors. The space is carried in the name
...
@dataclass
class Topics:
    """The recorded mapping between AETHRION channels and forum topics."""
    ...
    def channel_for(self, thread_id: int | None) -> str | None:
        """Which AETHRION channel a topic stands for, if any."""
